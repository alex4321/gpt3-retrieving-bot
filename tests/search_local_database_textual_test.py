...
def test_search_local_database_textual():
    temp_dir = tempfile.mkdtemp()
    try:
        search = SearchLocalDatabaseTextual(temp_dir, "english", 2)
        search.update(
            "Dixie Flatline",
            __DIXIE_FLATLINE_ARTICLE__
        )
        ...
        construct_search_documents = {search.split(" : ")[0] for search in construct_search}
        rockerboy_search_documents = {search.split(" : ")[0] for search in rockerboy_search}
        cortex_chip_search_documents = {search.split(" : ")[0] for search in cortex_chip_search}
        assert construct_search_documents == {"dixie flatline", "johnny silverhand"}
        assert rockerboy_search_documents == {"johnny silverhand"}
        assert cortex_chip_search_documents == set()
    finally:
